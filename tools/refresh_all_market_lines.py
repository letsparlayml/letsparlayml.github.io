...
def main() -> int:
    ap = argparse.ArgumentParser(description="Refresh NBA/NHL/MLB market lines for the site repo.")
    ...
    if not args.odds_api_key:
        print("[LINES] ERROR: ODDS_API_KEY not set. Cannot refresh NBA/NHL/MLB API odds.")
        return 1
    if not odds_fetch.exists():
        print(f"[LINES] ERROR: unified odds fetch script not found: {odds_fetch}")
        return 1

    cmd = [py, str(odds_fetch), '--api-key', args.odds_api_key, '--website-repo', str(repo)]
    run_step(cmd, 'Fetch NBA/NHL/MLB market lines from The Odds API')

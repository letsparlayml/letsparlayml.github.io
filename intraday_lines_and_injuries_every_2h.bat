...
if not exist "%TOOLS%\fetch_market_lines_the_odds_api.py" (
  echo ERROR: Missing %TOOLS%\fetch_market_lines_the_odds_api.py
  exit /b 1
)
...
"%PY%" "%TOOLS%\fetch_market_lines_the_odds_api.py" --api-key "%ODDS_API_KEY%" --website-repo "%REPO%"
if errorlevel 1 goto :fail

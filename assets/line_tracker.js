...
function buildTrackerEntries(games, archiveRows) {
  ...
  return (games || [])
    ...
    .map(game => {
      ...
      const displayDate = game.date || opening?.localDate || current?.localDate || opening?.utcDate || current?.utcDate || '';
      return {
        ...game,
        date: displayDate,
...
async function init() {
  ...
  if (dateSel) {
    ...
    const options = Array.from(dateSel.options).map(opt => opt.value);
